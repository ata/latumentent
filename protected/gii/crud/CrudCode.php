...
class CrudCode extends CCodeModel
{
	...
	{
		return lcfirst($this->_modelClass);
	}
	...
	{
		return "CHtml::activeLabelEx(\${$this->singularName},'{$column->name}')";
	}

	public function generateInputField($modelClass,$column)
	{
		if($column->type==='boolean')
			return "CHtml::activeCheckBox(\${$this->singularName},'{$column->name}')";
		else if(stripos($column->dbType,'text')!==false)
			return "CHtml::activeTextArea(\${$this->singularName},'{$column->name}',array('rows'=>6, 'cols'=>50))";
		else
		{
			...
			if($column->type!=='string' || $column->size===null)
				return "CHtml::{$inputField}(\${$this->singularName},'{$column->name}')";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "CHtml::{$inputField}(\${$this->singularName},'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))";
			}
		}
	}
	...
	{
		return "\$form->labelEx(\${$this->singularName},'{$column->name}')";
	}
	...
	public function generateActiveField($modelClass,$column)
	{
		...
		if($column->type==='boolean')
			...
			return "\$form->textArea(\${$this->singularName},'{$column->name}',array('rows'=>6, 'cols'=>50))";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\${$this->singularName},'{$column->name}')";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\${$this->singularName},'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength))";
			}
		}
	}
